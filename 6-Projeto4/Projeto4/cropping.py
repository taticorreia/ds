...
databowl = 'data/kaggle/'
kaggle_datafolder = 'data/kaggle/'
...
def test_nodules():
    ...
    for idx, patient in enumerate(test_df['id']):
        print(idx, patient)
        imgs = read_imgs('data/kaggle/stage1/' + patient)
        tr_nodules.append((get_filtered_nodules(imgs, unet)), patient)

    np.save('data/test_nodules.npy', np.array(tr_nodules))
...
def train_masks():
    ...
    for i in range(28):
        ...
        for idx, patient in train_df[batch_start:batch_end].iterrows():
            if idx % 5 == 0:
                print(i, idx, patient)
            slices = read_imgs('data/kaggle/stage1/' + patient['id'])
            masks = get_masks(slices, unet)
            nodules.append((masks, patient['id'], patient['cancer']))
            del masks
            del slices
        np.save('data/kaggle/masks/train_masks%d.npy' % i, np.array(nodules))
        te = time.time()
...
def crop_nodules_heatmap():
    ...
    for i in range(7):
        ...
        np.save('data/kaggle/masks/cropped_heatmap_nodules_heat2_dice40%d.npy' % i, np.array(nodules))
        te = time.time()
        print("Batch runtime:", te - ts)
